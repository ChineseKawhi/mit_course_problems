  ...
  def _compute_crossings(self, count_only):
    ...
    for event in self.events:
      ...
      if event_type == 'add':
        ...
        # cross_wires = []
        # temp = self.index.find2(wire.y1)
        # if(temp is not None):
        #   cross_wires.append(temp.value)
        #   temp = self.index.next_larger(wire.y1)
        # else:
        #   temp = self.index.find_min()
        #   while(temp is not None and temp.key<wire.y1):
        #     temp = temp.next_larger()
        # while(temp is not None and temp.key<=wire.y2):
        #   cross_wires.append(temp.value)
        #   temp = temp.next_larger()
        ...
        if count_only:
          result += self.index.range(KeyWirePairL(wire.y1), KeyWirePairH(wire.y2))
        else:
          cross_wires = []
          self.index.list(KeyWirePairL(wire.y1), KeyWirePairH(wire.y2), cross_wires)
          #print(cross_wires)
          for cross_wire in cross_wires:
            result.add_crossing(wire, cross_wire.wire)
      elif event_type == 'remove':
        self.index.remove(KeyWirePair(wire.y1, wire))
    #print(result.crossings)
