...
\reason
%%% PROBLEM 1(h) ANSWER START %%%
The area is bound, area decreases each step
}
\newcommand{\answerIu}{ \answer
%%% PROBLEM 1(u) ANSWER START %%%
\begin{quote}
  The recursion tree's height is n.
  And there are $3\times4^i$ nodes in level $i$.
  The asymptotic rendering tine for a node at level $i$
  is $\Theta(\frac{1}{9}^n)$, for $0 \le i < n$.
  So, the asymptotic rendering time at each level $i$
  is $\Theta(\frac{4}{9}^n)$, for $0 \le i < n$.
  As a result, the total asymptotic cost is the sum of 
  each level's asymptotic rendering time, which is $O(1)$
  
\end{quote}

%%% PROBLEM 1(u) ANSWER END %%%
\supplement
Each node renders one triangle.
There is no rendering work at level $n$.
}
