...
class PriorityQueue:
    ...
    def append(self, key):
        ...
        self._heap_top(len(self.queue)-1)
        #self.min_index = None
    ...
    def _heap_top(self, idx):
        ...
        if(parent_idx >= 0 and self.queue[parent_idx] > self.queue[idx]): # if not, we're done
    ...
    def _heap_down(self, idx):
        ...
        if(not smaller_idx == idx): # if equals, we're done
            ...
            self.queue[idx], self.queue[smaller_idx] = self.queue[smaller_idx], self.queue[idx]
    ...
    def __get_smaller_child_idx(self, idx):
        ...
        smaller_idx = idx
        if(l < len(self.queue) and self.queue[l] < self.queue[smaller_idx]):
            smaller_idx = l
        if(r < len(self.queue) and self.queue[r] < self.queue[smaller_idx]):
            smaller_idx = r
        return smaller_idx
