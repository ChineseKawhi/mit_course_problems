...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        while(len(q)>0):
            ...
            if cur is destination:
                break
            for node in cur.node.adj:
                dis = cur.distance + weight(cur.node, node) 
                if node.queue_key is None:
                    node.queue_key = NodeDistancePair(node, dis)
                    q.insert(node.queue_key)
                    node.prev = cur.node
                elif dis < node.queue_key.distance:
                    node.prev = cur.node
                    node.queue_key.distance = cur.distance + weight(cur.node, node)
                    q.decrease_key(node.queue_key)
        cur = destination
        path = []
    ...
    ## Fist Version: correct but not beautiful
    # def dijkstra(self, weight, nodes, source, destination):
    #     """Performs Dijkstra's algorithm until it finds the shortest
    #     path from source to destination in the graph with nodes and edges.
    #     Assumes that all weights are non-negative.
    ...
    #     res = {}
    #     for node in nodes:
    #         node.visited = False if node != source else True
    #         node.prev = None
    #         pair = NodeDistancePair(node, float('inf') if node != source else 0)
    #         q.insert(pair)
    #         dic[node] = pair
    #     while(len(q)>0):
    #         cur = q.extract_min()
    #         cur.visited = True
    #         res[cur.node] = cur.node.prev
    #         if cur is destination:
    #             break
    #         for node in cur.node.adj:
    #             pair = dic[node]
    #             if cur.distance + weight(cur.node, node) < pair.distance:
    #                 node.prev = cur.node
    #                 pair.distance = cur.distance + weight(cur.node, node)
    #                 q.decrease_key(pair)
    #     cur = destination
    #     path = [cur]
    #     while(cur is not None):
