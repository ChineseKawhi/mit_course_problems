...
\li $purchase = \proc{Stock-Table}(\total, \stockCount, \priceStart, \priceEnd, \limit)$
\li \Return \proc{Stock-Result}(\total)
\end{codebox}

\begin{codebox}
  \Procname{$\proc{Stock-Table}
      (\total, \stockCount, \priceStart, \priceEnd, \limit)$}
  ...
  \li				\If $\limit[\stock] > 0$ \Then
  \li       $\current \gets \current + \profit[\leftover, \stock]$ 
  \li       \Else 
  \li       $\current \gets \current + \profit[\leftover, \stock-1]$ \End
  \li				\If $\profit[\cash, \stock] < \current$ \Then
  \li					$\profit[\cash, \stock] = \current$
  \li					$\purchase[\cash, \stock] = \const{True}$
  \li					$\limit[\stock] \gets \limit[\stock] - 1$
           \End
        \End
      \End
    \End
